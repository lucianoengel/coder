...
function runNative(repoDir, config = {}) {
  return runPpcommitNative(repoDir, { blockSecrets: false, ...config });
}

function runBranch(repoDir, baseBranch, config = {}) {
  return runPpcommitBranch(repoDir, baseBranch, {
    blockSecrets: false,
    ...config,
  });
}

function runAll(repoDir, config = {}) {
  return runPpcommitAll(repoDir, { blockSecrets: false, ...config });
}
...
test("ppcommit: skip via config", async () => {
  ...
  const r = await runNative(repo, { skip: true });
...
test("ppcommit: treatWarningsAsErrors upgrades warnings", async () => {
  ...
  const r = await runNative(repo, { treatWarningsAsErrors: true });
  assert.equal(r.exitCode, 1);
...
test("ppcommit branch: no files changed since base", async () => {
  ...
  const r = await runBranch(repo, "main");
  assert.equal(r.exitCode, 0);
...
test("ppcommit branch: skip via config", async () => {
  ...
  run("git", ["add", "a.js"], repo);
  run("git", ["commit", "-m", "add a.js"], repo);
  const r = await runBranch(repo, "main", { skip: true });
  assert.equal(r.exitCode, 0);
  assert.match(r.stdout, /skipped/i);
});
...
test("ppcommit branch: invalid base ref is an error (does not silently succeed)", async () => {
  const repo = makeRepoWithMainBranch();
  const r = await runBranch(repo, "definitely-not-a-real-branch");
...
test("ppcommit: gitleaks missing from PATH produces actionable error", async () => {
  ...
  try {
    await assert.rejects(
      () => runPpcommitNative(repo, { blockSecrets: true }),
